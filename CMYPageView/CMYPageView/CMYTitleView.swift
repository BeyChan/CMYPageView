...
// MARK: TitleView协议定义
protocol CMYTitleViewDelegate: class {
    func titleView(_ title: CMYTitleView, selectedIndex index: Int)
}

class CMYTitleView: UIView {
    // MARK: 对外属性
    weak var delegate: CMYTitleViewDelegate?
    ...
    fileprivate lazy var splitLineView: UIView = {
        let splitV = UIView()
        splitV.backgroundColor = UIColor.red
        let h: CGFloat = 0.5
        splitV.frame = CGRect(x: 0, y: self.frame.height - h, width: self.frame.width, height: h)
        return splitV
    }()
    ...
    // MARK: 自定义构造函数
    init(frame: CGRect, titles: [String], style: CMYTitleStyle) {
